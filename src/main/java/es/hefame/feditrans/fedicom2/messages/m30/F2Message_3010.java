...
public class F2Message_3010 extends F2Message{
	...
	public F2Message_3010(String data, int version) {
		super(data, version, MessageType.CABECERA_ABONO);
		
		int idx = 4;
		codCli = data.substring(idx, idx+=16);
		numAlbaran = data.substring(idx, idx+=20);
	...
	public JSONObject toFedicom3() throws Exception {
		...
		if (numAlbaran.trim().length() > 0) 
			root.put("numeroPedidoOrigen", numAlbaran.trim() + "-" + System.currentTimeMillis() + "-" + Math.random()); 
		else 
	...
	public String toString() {
		return super.toString() + " [codCli=<" + codCli + ">, numAlbaran=<" + numAlbaran + ">, tipoPed=<" + tipoPed + ">, condi=<"
